...
dtable_latex(dtable(df, type = "date", glist = gl3, w = vikt, comp = "across"),
             ...
             caption = paste("Three 'weighted' groups described and compared",
                             "'across' for type \\texttt{date}. Notice that the",
                             " default functions ignore weighting (so there is no",
                             " weighting but no warning)."))
@
...
Here we also pass \texttt{attr = FALSE} to \texttt{attr2text} to supress the
inclusion of attributes in the table.
