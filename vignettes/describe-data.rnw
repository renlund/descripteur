...
This function calculates the standardized difference for all applicable variables.
...
\begin{itemize}
\item \texttt{dtable\_prune} remove columns by name or index with the possibility
  of saving the (unique) information in an attribute.
