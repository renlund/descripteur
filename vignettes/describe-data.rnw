...
categorical variables to have this option\ldots \\ \textbf{Should this be
  removed?}
...
One might want to move the info column to the 'info' attribute to get it to
print nicely below the table in a \LaTeX-table. This is done by 'pruning' the
table with option \texttt{info = TRUE} which saves the (unique) pruned
information.  Also, there is still some info unexplained so we add something to
the 'info' attribute.
<<results = 'asis'>>=
attr(dt, "info") <- c(attr(dt, "info"), "[n] is missing count")
tmp1 <- dtable_prune(dt, rm = "variable", info = FALSE)
tmp2 <- dtable_prune(tmp1, rm = "info", info = TRUE)
dtable_latex(tmp2, label = "tab:info",
             caption = "Compact table with all types")
@
...
dtable_latex(tmp3, label = "tab:info",
...
This function simply calculated the standardized difference for all applicable variables.

<<results = 'asis'>>=
ds <- dtable_std(data = df, glist = "b1", guide = dtg)
tmp <- dtable_subset(x = ds, subset = ds$variable != "binary 1",
                     all.attr = TRUE)
dtable_latex(tmp, label = "tab:std", format = TRUE,
...
This simple description of missing provides counts and percentages for variables in a data
frame (regardless of type).

<<results = 'asis'>>=
dm <- dtable_missing(data = df, glist = gl, guide = dtg)
...
\begin{itemize}
\item \texttt{dtable\_prune} remove columns by name of index with the possibility
  of saving the (unique) information in an attribute instead.
\item \texttt{dtable\_rbind} and \texttt{dtable\_cbind} which are analogous to
  ...
  attributes are not mergeable and the results will generally inherit these
  from the first argument (and thus may not be correct for the combined object).
\item \texttt{dtable\_fnote} a function to turn one column into a footnote of another.
