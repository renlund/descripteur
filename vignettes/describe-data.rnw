...
\item A function that needs the variable name can use \texttt{xname} as this is
  passed internally to the function (and hence do not use that argument name for
  something else)
...
functions, if so they can have a ``second'' prefix begin the first letter of the
type, so there is e.g.\ \texttt{d\_rstd} and \texttt{d\_bstd} which calculated
the standard difference for \texttt{real} and \texttt{bnry}, respectively.

Type \texttt{?d\_real} to get a list of all describers applicable to the
\texttt{real} type.
...
dtg$label <- c("binary 1", "bengt", "bio", "bam",
               "cats are fun", "cool",
...
dtable_latex(dtable(df, type = "catg", glist = gl, guide = nobnry), format = T,
             caption = paste("Two groups described and compared",
                             "for type \\texttt{catg} (including",
                             " binary variables)."))
@
...
\texttt{dtable\_list\_latex} tries to alleviate this. The latter function tries to
make intelligent use of \texttt{sprintf} to create good labels and captions (but
you could also give your own vector of labels and captions).
...
dtable_list_latex(dtl, label = "tab:foo-%s",
...
dtable_list_latex(dtl, label = "tab:bar-%s",
...
Still experimental, and possibly too cumbersome\ldots

<<eval = FALSE>>=
dt <- dtables(data = df, useNA = "ifany")
attributes(dt)
dt2 <- footnote(dt, text = "info")
attributes(dt2)
dtable_format(dtables(data = df, useNA = "ifany", glist = "b1"))
@
...
\subsection{Simple description of attributes and missing, respectively}
\texttt{dtable\_missing} provides counts and percentages for variables in a data
frame (regardless of type). \texttt{dtable\_attr} returns some attributes in a
tidy format. Both these have grouping.

<<>>=
dm <- dtable_missing(df, glist = gl)
@
...
<<results = 'asis'>>=
dtable_latex(dm, format = TRUE, guide = dtg,
             caption = "Information on missing",
             bling.param = list(sep = c(". ", "\\newline ")))
@

\subsection{Finer control of output}

\texttt{dtable\_latex} can pass arguments to \texttt{attr2text} (which works on
top of \texttt{dtable\_attr}) to select which attributes to attach and (in a very
limited way) how these are presented.

\vspace{1cm}
\noindent Presentation may appear here at some point\ldots
